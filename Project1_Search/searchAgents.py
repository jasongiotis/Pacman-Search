...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        return [self.startingPosition,[]] ##returning the cordinates and a list of the already visited corners
    ...
    def isGoalState(self, state):
        ...
        return len(state[1])==4 ## goal state when we reach every corner
    ...
    def expand(self, state):
        ...
        for action in self.getActions(state):
            ...
            dx, dy = Actions.directionToVector(action)
            nextx, nexty = int(x + dx), int(y + dy)
            illegal_move = self.walls[nextx][nexty]       ## checking if a move is legal
            if not illegal_move:
                next_corners = list(state[1])
                if (nextx, nexty) in self.corners:   ##checking if the child is a non visited  corner
                    if (nextx, nexty) not in next_corners:
                        next_corners.append((nextx, nexty))
                child = (((nextx, nexty), next_corners), action, 1)
    ...
    def getNextState(self, state, action):
        ...
        nextx, nexty = int(x + dx), int(y + dy)
        "*** YOUR CODE HERE ***"
        illegal_move=self.walls[nextx][nexty] ##just returning the next state if the move is legal
        if not illegal_move:
            return (nextx,nexty)
        util.raiseNotDefined()
...
def cornersHeuristic(state, problem):
    ...
    while(len(notvisited)!=0): ##finding the distance to the nearest corner and then from that nearaest corner to the next. Till we run out of corners
        mindist=util.manhattanDistance((x,y),notvisited[0])
        newx,newy=notvisited[0]
        for corner in notvisited:
            if util.manhattanDistance((x,y),corner)<mindist:
                newx,newy=corner
                mindist=util.manhattanDistance((x,y),corner)
        notvisited.remove((newx,newy))
        x=newx
        y=newy
        cost=cost+mindist ##each time we add the distance of the nearest corner
    return cost
...
def foodHeuristic(state, problem):
    ...
    for i in (range(len(foodpos))):  # it expands only 4100 and something nodes).We can do the same with
        foodcost= mazeDistance(position, foodpos[i],problem.startingGameState)  # the cornersheuristis using manhattanheuristic but for food and corners
        if(foodcost>max):
            max=foodcost
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        actions = search.breadthFirstSearch(problem) ## We use BFS to find the closest dot
        return actions
        util.raiseNotDefined()
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        min=(util.manhattanDistance(state,foodList[0]))
        minfood=foodList[0]
        for food in foodList: ##we find the closest food to our state
            if util.manhattanDistance(state, food)<min:
                min = (util.manhattanDistance(state, food))
                minfood=food
        return state == minfood  ##we check if we are on it
